...
// ToPngImage 将Go的Image转为LCL的 TPngImage
// 返回的Png对象用完记得Free掉
...
// ToJPEGImage 将Go的Image转为LCL的 TJPEGImage
// 返回的jpg对象用完记得Free掉
...
// ToGIFImage 将Go的Image转为LCL的 TGIFImage
// 返回的gif对象用完记得Free掉
...
func toBitmap(width, height int, pix []uint8, bmp *lcl.TBitmap) error {
	if len(pix) == 0 {
